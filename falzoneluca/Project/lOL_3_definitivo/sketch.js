...
var counter = 0;
// variabile per il display dell'ultimo oggetto riconosciuto
var ultimoOggettoRiconosciuto = " ";
// variabile per la dimensione dell'interfaccia
var dimensioneInterfaccia = 60;
...
function gotResults(error, results) {
    if (error) {
        ...
        ultimoOggettoRiconosciuto = results[0].label;
        // riconoscimento di oggetti (espandibile)
        if ((results[0].label == "mascara" || results[0].label == "lenti") &&
        ...
        else if ((results[0].label == "pinzette" || results[0].label == "pinze") &&
                 (results[0].confidence >= 0.7)) {
                     zoom.tipo = "naso";
        }
        else {
            zoom.tipo = "none";
            ultimoOggettoRiconosciuto = " ";
        }
    }
}
...
function draw() {
    ...
    textSize(dimensioneInterfaccia);
    ...
    if (posePronto) {
        ...
        if (poses.length > 0) {
            // prende come centro del volto il naso
            centroX = poses[0].pose.keypoints[0].position.x;
            centroY = poses[0].pose.keypoints[0].position.y;
            // se lo zoom è di tipo bocca
            if (zoom.tipo == "bocca") {
                // sposta il centro più in basso
                centroY += 50;
            }
            else if (zoom.tipo == "occhi") {
    ...
    text(ultimaParola, larghezza / 2, altezza - 40);
    textAlign(RIGHT);
    text(ultimoOggettoRiconosciuto, larghezza - 50, 80);
    ...
    if (oggetti.length > 0 && allenato == false) {
        ...
        for (var i = 0; i < oggetti.length; i++) {
            ...
            fill(255);
            textSize(30);
            text(oggetti[i].nome, larghezza - 50 * 3 - 40, 40 * (i + 1) + 30);

            // disegna la barra di caricamento
            if (oggetti[i].numero != 0) {
                fill(255, 0, 0);
                rect(larghezza - 50 * 3 - 20, 40 * (i + 1), (50 - oggetti[i].numero) * 3, 30);
            }
            // disegna la barra caricata
            else {
                fill(0, 255, 0);
                rect(larghezza - 50 * 3 - 20, 40 * (i + 1), 50 * 3, 30);
            }
        }
    }
...
function analizzaAudio() {
    ...
    if (modalitaAscolto) {
        ...
        if (parolaChiave == "riconosci" || parolaChiave == "impara") {
        ...
        else if (parolaChiave == "start" || parolaChiave == "inizia" || parolaChiave == "parti" || parolaChiave == "riparti") {
            // attiva lo zoom
            attivaZoom = true;
        }
